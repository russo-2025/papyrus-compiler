...
fn (mut g Gen) gen(file &ast.File) {
	...
	if g.sym.parent_idx == 0 {
		g.server_ts_h.writeln("\tclass ${g.obj_name} {")
	}
	else {
		g.server_ts_h.writeln("\tclass ${g.obj_name} extends ${g.parent_obj_name} {")
	}
	
	g.server_ts_h.writeln("\t\tstatic From(formId: number): ${g.obj_name} | null")
...
fn (mut g Gen) gen_impl_fn(sym &ast.TypeSymbol, func &ast.FnDecl) {
	...
	if func.is_global {
		g.server_main_cpp.writeln("\t\tVarValue res = ${g.gen_vm_fn_impl_name(sym.obj_name, func.name)}(VarValue::None(), args);")
	}
	else {
		g.server_main_cpp.writeln("\t\tVarValue res = ${g.gen_vm_fn_impl_name(sym.obj_name, func.name)}(self, args);")
	}
...
fn (mut g Gen) gen_ts_h_fn(sym &ast.TypeSymbol, func &ast.FnDecl) {
	...
	if func.is_global {
		g.server_ts_h.writeln("\t\tstatic ${func.name}(${g.temp_args.str()}): ${g.get_ts_type_name(func.return_type)}")
	}
	else {
		g.server_ts_h.writeln("\t\t${func.name}(${g.temp_args.str()}): ${g.get_ts_type_name(func.return_type)}")
	}
}