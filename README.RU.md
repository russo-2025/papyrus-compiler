...
- [Отличия от оригинального компилятора](#отличия-от-оригинального-компилятора)
- [Сборка](#сборка)
...
- `-no-debug-info`: Исключает номера строк исходного кода и время модификации из скомпилированных .pex файлов. Информация об отладке включена по умолчанию.
- `-verbose`: ...
...
## Отличия от оригинального компилятора

У этого компилятора есть несколько известных отличий от оригинального компилятора Papyrus от Bethesda. Часть отличий сделана намеренно, а часть описывает поведение, которое сейчас воспроизводится не полностью.

### 1. Многострочные строковые литералы

Перенос строки, поставленный прямо внутри строкового литерала, сохраняется как перенос в итоговой строке. Принимаются и Unix (`LF`), и Windows (`CRLF`) концы строк, при этом они сохраняются как один `LF` — поэтому содержимое строки не зависит от стиля переносов в исходном файле:
...
Оригинальный компилятор не позволяет строковому литералу занимать несколько строк.

### 2. Регистр строк во время выполнения

Оригинальный компилятор Bethesda может менять регистр строковых литералов при компиляции скрипта. Это не видно в исходном `.psc` файле, но может изменить значение, которое переменная получит во время выполнения.

Papyrus не различает регистр в именах переменных, функций, свойств и типов. Оригинальный компилятор также переиспользует уже встреченный текст без проверки, отличается ли регистр букв. Из-за этого строковый литерал может случайно получить регистр более раннего имени переменной или более ранней строки.
...
У оригинального компилятора переменные могут получить такие значения:

```text
s1 == "FooBar"
s2 == "FooBar" ; а не "foobar"
barfoo == "barfoo" ; а не "BarFoo"
s3 == "barfoo" ; а не "BarFoo"
```

В этом примере `"foobar"` считается тем же текстом, что и уже встреченное `"FooBar"`, поэтому побеждает более раннее написание. Литерал `"BarFoo"` также может пострадать из-за локальной переменной `barfoo`, потому что компилятор уже видел это имя и считает его тем же текстом без учета регистра.

Этот компилятор сохраняет регистр строковых литералов ровно так, как он написан в исходном коде. Тот же скрипт сохранит ожидаемые значения:
...
Это влияет на:
- скрипты, которые сравнивают строки с учетом регистра;
- строки, которые используются как ключи, теги, ID, имена файлов, JSON-ключи, текст интерфейса или значения, передаваемые другим инструментам и плагинам;
- отладочный вывод и логи, где важно точное написание.

Для обычного Papyrus-кода это обычно не важно, потому что имена в Papyrus нечувствительны к регистру. Это важно, когда строка используется как данные и скрипт ожидает, что она сохранит ровно тот регистр, который написан в `.psc` файле. В таком случае этот компилятор ведет себя предсказуемее оригинального.
